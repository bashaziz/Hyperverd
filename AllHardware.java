...
public class AllHardware extends JFrame {
  ...
  private Connection dbconnection = null;
    ...
    public AllHardware() {
		...
		try
		{
			...
			CallableStatement cs = dbconnection.prepareCall("{call getAllHardware()}");
		...
		if (row == 0)
		{
			ProjectNew.Log("", ProjectNew.ActivityType.NORMAL, "No Devices have been registered!!!", ProjectNew.getDelayTime());
			setVisible(false);
		}
		if (row != 0)
		{
        ...
        gradesLabel = new JLabel("All Devices");
        gradesLabel.setFont(new Font("Dialog", Font.BOLD, 16));
		String col[] = {"Type_ID", "Brand_ID", "Model", "Serial Number", "Barcode"};
        reportModel = new DefaultTableModel(col, row);
		deviceTable = new JTable(reportModel);
		...
		if (row == 0)
		{
			ProjectNew.Log("", ProjectNew.ActivityType.NORMAL, "No Device has been Checked-in Today!!!", ProjectNew.getDelayTime());
			setVisible(false);
		}
		if (row != 0)
		{
  ...
  private void viewReport()
  {
    try
    {
      ...
      while (result.next())
      {	        
		deviceTable.setValueAt( result.getString("Type_ID"), count, 0);
		deviceTable.setValueAt( result.getString("Brand_ID"), count, 1 );
		deviceTable.setValueAt( result.getString("Model"), count, 2 );
		deviceTable.setValueAt( result.getString("SerialNumber"), count, 3 );
		deviceTable.setValueAt( result.getString("Barcode"), count, 4 );
		count++;
      }
    }
	catch (SQLException xcp)
    ...
    private void printdeviceTable() 
	{
        ...
        try 
		{
            boolean complete = deviceTable.print(mode, header, footer,
