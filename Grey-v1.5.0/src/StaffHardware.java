...
public class StaffHardware extends JFrame 
{
  ...
  private Connection db = null;
    ...
    public StaffHardware() {
        super("Staff Devices");
		int row = 0;
		try
		{
			Connection dbconnection = helper.getConnection();
			CallableStatement cs = dbconnection.prepareCall("{call getStaffEquipment()}");
        ...
        gradesLabel = new JLabel("Staff Devices");
        gradesLabel.setFont(new Font("Dialog", Font.BOLD, 16));
		String col[] = {"Staff ID", "Full Name of Staff", "Device in Possession of"};
		...
		if (row == 0)
		{
			ProjectNew.Log("", ProjectNew.ActivityType.NORMAL, "No Devices have been assigned to any Staff!!!", ProjectNew.getDelayTime());
			setVisible(false);
		}
		if (row != 0)
		{
  ...
  private void viewReport()
  {
    try
    {
      ...
      {	        
		...
		reportTable.setValueAt( result.getString("Full Name"), count, 1 );
		reportTable.setValueAt( result.getString("Equipment(s) In Possession"), count, 2 );
		count++;
      }
    }
	catch (SQLException xcp)
