...
public class NewUpdateStaff extends JFrame 
  {
  ...
  //ADD STAFF THINGS....
  private static final String staffTitle = "Update Staff Photo";
  ...
  private void updateStaff()
  {
    try
    {
	  ...
	  CallableStatement csu = dbconnection.prepareCall("{call getStaff(?)}");
      csu.setString(1, id);
	  ResultSet result = csu.executeQuery();

	  CallableStatement csu1 = dbconnection.prepareCall("{call getDevice(?)}");
      csu1.setString(1, id);
	  ResultSet result1 = csu1.executeQuery();
	  
	  while (result.next() && result1.next())
	  {
		  staffFirstNameTextField.setText(String.format("%s", result.getString("FirstName")));
		  staffLastNameTextField.setText(String.format("%s", result.getString("SurName")));
		  staffSexDropDown.setSelectedItem(String.format("%s", result.getString("Sex")));
		  staffPracticeDropDown.addItem(result.getString("Practice_ID"));
		  staffPracticeDropDown.setSelectedItem(result.getString("Practice_ID"));
		  staffDeviceDropDown.addItem(result1.getString("SerialNumber"));	
		  staffDeviceDropDown.setSelectedItem(result1.getString("SerialNumber"));	
		  lblPhoto.setIcon(new ImageIcon(result.getBytes("Image")));	
	  }  
	  if (staffFirstNameTextField.getText() != null)
		{
		  staffIdTextField.setEnabled(false);
		  staffFirstNameTextField.setEnabled(false);
		  staffLastNameTextField.setEnabled(false);
		  staffDeviceDropDown.setEnabled(false);
		  staffSexDropDown.setEnabled(false);
		  staffPracticeDropDown.setEnabled(false);
		  ProjectNew.Log("", ProjectNew.ActivityType.NORMAL, "Staff Identified!!!", ProjectNew.getDelayTime());
		}
	  if (csu.executeUpdate() == 0 && csu1.executeUpdate() == 0)
	  {
		ProjectNew.Log("", ProjectNew.ActivityType.ERROR, "Invalid Staff ID!!!", ProjectNew.getDelayTime());
	  }
    }
  ...
  private void updateNewStaffSend()
  {
	if (staffIdTextField.getText().equals("") || staffFirstNameTextField.getText().equals("") || staffLastNameTextField.getText().equals("") || staffSexDropDown.getSelectedItem() == null || staffPracticeDropDown.getSelectedItem() == null || staffDeviceDropDown.getSelectedItem() == null || lblPhoto.getIcon() == null)
	{
	  ...
	  ProjectNew.Log("", ProjectNew.ActivityType.WARNING, "Please Type in Staff ID and hit Enter!!!", ProjectNew.getDelayTime());
	}
    ...
    try
    {
	  ...
	  cbs.setObject(2, staffDeviceDropDown.getSelectedItem().toString().split(" ")[0]);
	  
	  if (cs.executeUpdate() >= 1 && cbs.executeUpdate() >=1)
		{
			ProjectNew.Log("", ProjectNew.ActivityType.ERROR, "Staff has successfully been updated!!!", ProjectNew.getDelayTime());
			staffIdTextField.setText(null);
			staffFirstNameTextField.setText(null);
			staffLastNameTextField.setText(null);
			staffSexDropDown.setSelectedItem(null);
			...
			staffFirstNameTextField.setEnabled(true);
			staffLastNameTextField.setEnabled(true);
			staffSexDropDown.setEnabled(true);
			staffDeviceDropDown.setEnabled(true);
			staffPracticeDropDown.setEnabled(true);
			setVisible(false);
		}
    }
    catch (SQLException e)
    ...
    {
	  //Log("", ActivityType.ERROR, "Could not establish connection to the Database Server! Tried 1 time(s) : Giving up...", ProjectNew.getDelayTime());
      e.printStackTrace();
    }
  }
  ...
  private void addStaffCall()
  {
	...
	staffIdTextField.addActionListener(new ActionListener()
	{
    ...
    JButton staffUpdateButton = new JButton("Update Photo");
	staffUpdateButton.addActionListener(new ActionListener()
	{
		public void actionPerformed(ActionEvent e)
		{
			preparePhoto(true);
	...
	c.gridy = 6;
    topPanel.add(staffUpdateButton, c);
	...
	{
		PhotoCapture1 pc = new PhotoCapture1(this);
		pc.setVisible(true);
    }
