  ...
  ConnectionHelper helper = new ConnectionHelper();
   ...
   /**
    ...
    public AllStaff() {
		...
		String col[] = {"Staff ID", "Firstname", "Surname", "Gender", "Practice", "Photo"};
        reportModel = new DefaultTableModel(col, row);
		reportTable = new JTable(reportModel);
        reportTable.setFillsViewportHeight(true);
        reportTable.setRowHeight(24);
		reportTable.setDefaultRenderer(JComponent.class, new ClientsTableRenderer());
  ...
  private void viewReport()
  {
    try
    {
      ...
      {	        
		...
		//photoIcon = new ImageIcon(result.getBytes("Image"));
		//lblPhoto.setIcon(photoIcon);
		//photoPanel.add(lblPhoto);
		//reportTable.setValueAt( photoPanel, count, 5);
		count++;
      }
    }
	catch (SQLException xcp)
	...
	public class ClientsTableRenderer extends JPanel implements TableCellRenderer {
		public Component getTableCellRendererComponent( final JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
			...
			if (col.equals("Photo")) {
				...
				JButton button = new JButton("view photo");
